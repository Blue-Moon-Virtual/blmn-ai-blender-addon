...
def select_link_function(self, tag):
    ...
    # Blue Moon AI: prefer the release-attached `blmn-ai-blender-addon*.zip`
    # asset (the exact binary the release workflow builds and blmn.ai/blender
    # serves) over GitHub's auto-generated source zipball, so an in-app update
    # installs the same vetted package as a fresh manual download. Falls back to
    # the source zipball if no matching asset is attached to the release.
    assets = tag.get("assets") if isinstance(tag, dict) else None
    if assets:
        for asset in assets:
            name = (asset.get("name") or "").lower()
            url = asset.get("browser_download_url")
            if url and name.startswith("blmn-ai-blender-addon") and name.endswith(".zip"):
                link = url
                break
...
def register(bl_info):
    ...
    updater.repo = "blmn-ai-blender-addon"

    # updater.addon = # define at top of module, MUST be done first

    # Website for manual addon download, optional but recommended to set.
    updater.website = "https://blmn.ai/blender"

    # Addon subfolder path: the package folder inside our release zip
    # (`blmn-ai-blender-addon.zip` contains `blmn_ai/...`), so the updater
    # copies the contents of `blmn_ai/` over the installed add-on.
    updater.subfolder_path = "blmn_ai"
    ...
    # Blue Moon AI: end users only ever install tagged releases, not branches.
    ...
    # Blue Moon AI: use GitHub Releases so the updater installs the
    # release-attached binary (see select_link_function) and can surface notes.
